...
<!-- 						</label> -->
						...
						<label class="btn btn-flat btn-outline-dark">
							<input type="radio" name="options" id="qna" autocomplete="off" onclick="location.href='myqnaList.do'" checked>1:1문의
						</label>
						<label class="btn btn-flat btn-outline-dark">
				...
				<div style="width: 80%; margin: 0 auto;">
					<div class="card-body table-responsive p-0">
							...
							<tbody>
								...
								<c:if test="${fn:length(qnaList) > 0}">
									<c:forEach var="item" items="${qnaList}" varStatus="status">
										<tr onclick="location.href='myqnaInfo.do?qnaId=${item.qnaId}'">
											<td>${item.qnaType}</td>
											<td>${item.qnaTitle}</td>
											<td>${item.qnaDate}</td>
						...
						<div class="text-right"  style="margin-top: 10px;">
							<button type="button" class="btn btn-sm btn-dark" id="btnWithdraw" onclick="location.href='qnaRegister.do'">글쓰기</button>
						</div>
					</div>
