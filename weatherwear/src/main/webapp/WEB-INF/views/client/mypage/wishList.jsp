			...
			<section id="contact" class="contact mb-2">
				...
				<div class="text-center">
					<div class="btn-group btn-group-toggle col-sm-10" data-toggle="buttons">
						...
						<label class="btn btn-flat btn-outline-dark">
							<input type="radio" name="options" id="wish" autocomplete="off" onclick="location.href='mywishList.do'" checked>관심상품
						</label>
						<label class="btn btn-flat btn-outline-dark">
						...
						<table class="table table-hover text-nowrap" style="table-layout: fixed;">
							...
							<tbody>
								<c:if test="${fn:length(wishList) == 0}">
									<tr>
										<td colspan="7">보관하신 상품 내역이 없습니다.</td>
									</tr>
								</c:if>
								<c:if test="${fn:length(wishList) > 0}">
									<c:forEach var="item" items="${wishList}" varStatus="status">
										<tr>
											...
											<td>
												...
												<br><button class="btn btn-sm btn-flat btn-outline-secondary col-sm-4 delWish">삭제</button>
											</td>
										</tr>
									</c:forEach>
								</c:if>
